...
export default class AreaContainer extends React.Component {
    render() {
        ...
        return <div className={classnames({container:true,hide:show})}>
            {item && item.length > 0 && item.map((child, i)=>(<a href="javascript:void(0)" className={classnames({active:child.name==currentItem})} key={i} onClick={getCell&&getCell.bind(null,child.name,child.cell)}>{child.name}</a>))}
        </div>
    }
}